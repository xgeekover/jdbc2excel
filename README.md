...
- **자동 결과 검증**: 저장된 엑셀을 다시 열어 DB 조회 결과와 셀 단위로 대조 (불일치 시 위치 보고 + 종료 코드 1)
...
| `verify` | | `true` | 저장 후 엑셀을 다시 열어 DB 결과와 셀 단위 대조 |
...
### 자동 결과 검증 (`verify`, 기본 활성)

엑셀 저장 직후 파일을 다시 열어(스트리밍 read_only 모드) DB에서 받은 결과와
**헤더·전체 데이터 행을 셀 단위로 대조**한다.

```
10:29:04 INFO  [h2_e2e] 엑셀 검증 통과: 시트 3개, 총 8행 셀 단위 일치
```

- 불일치가 있으면 `'시트명' 2행 3열: 기대 '★' ≠ 실제 '?'`처럼 위치를 특정해 보고하고 종료 코드 1을 반환한다.
- 행 누락·추가 행·예상 밖의 값도 감지한다.
- xlsx 저장 규칙상 정당한 변환만 허용한다: 빈 문자열/NULL → 빈 셀, `date` → 자정
  `datetime`(엑셀 날짜 직렬화), 제어문자 제거(엑셀 스펙상 저장 불가), 숫자의 IEEE double
  정밀도(xlsx 형식 한계). 그 외 모든 차이는 불일치로 보고한다.
- 대용량 결과에서 검증 시간을 아끼려면 `"verify": false`로 끌 수 있다.

### 유니코드가 깨지지 않는 이유

전 구간에서 코드페이지 변환이 한 번도 일어나지 않는다:

1. **JDBC → 파이썬**: 자바 String(UTF-16)이 JPype를 통해 파이썬 `str`(유니코드 코드포인트)로
   직접 매핑된다. 바이트 인코딩/디코딩 단계 자체가 없다.
2. **파이썬 → xlsx**: xlsx 내부는 UTF-8 XML이고 openpyxl이 전 문자를 그대로 기록한다.
3. **script.sql**: UTF-8(BOM 허용)로 읽으므로 한글 탭 이름·쿼리 안의 한글 리터럴도 안전하다.

값이 변형되는 경우는 딱 두 가지이며 모두 의도된 것이다:

- **제어문자 제거**: 엑셀 스펙상 저장 불가능한 제어문자(0x00-0x08 등)만 제거. 한글·한자·
  이모지·수학기호·통화기호는 모두 보존된다.
- **수식 주입 방지**: `=`로 시작하는 DB 값은 수식이 아닌 **텍스트**로 강제 저장한다.
  (미처리 시 엑셀이 계산식으로 해석해 원본과 다르게 표시됨)

이 보장은 `tests/run_tests.py`의 회귀 테스트(이모지·한자·키릴·그리스 문자 라운드트립,
훼손 파일 감지 등 18건)로 상시 검증된다:
